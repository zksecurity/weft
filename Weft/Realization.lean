...
theorem mem_support_dist_call {ι : Interface} (M : Model ι .ideal PMF) {α : Type} (r : Req ι .ideal)
    ...
    p ∈ (dist M (.call r k)).support ↔
      ∃ z ∈ (M.step r).support, ∃ q ∈ (dist M (k z.1)).support,
        p = (q.1, ⟨r.op, (ι.dom r.op).blank r.args, (ι.cod r.op).blank z.1, z.2⟩ :: q.2) := by
  rw [dist_call]
  ...
  | call r k' ih =>
    ...
    | call _ _ hr hk' =>
      exact .call r _ hr fun z hz => ih z.1 (hk' z hz) fun q hq =>
        hk (q.1, ⟨r.op, (ι.dom r.op).blank r.args, (ι.cod r.op).blank z.1, z.2⟩ :: q.2) ((mem_support_dist_call M r k' _).2 ⟨z, hz, q, hq, rfl⟩)
  | look c k' ih =>
    cases hc with
...
structure Realization (F : Functionality) (fs : Hybrid) where
  ...
  real : ∀ r, Pre r → dist fs.model (impl .ideal r) = (do
    ...
    let s ← Sim ⟨r.op, (F.ops.dom r.op).blank r.args, (F.ops.cod r.op).blank y, d⟩
    pure (y, s))
...
theorem real (g : Realizations fs gs) (r : Req fs.ops .ideal) (h : g.Pre r) :
    dist gs.model (g.impl .ideal r) = (do
      ...
      let s ← g.Sim ⟨r.op, (fs.ops.dom r.op).blank r.args, (fs.ops.cod r.op).blank y, d⟩
      pure (y, s)) := by
  obtain ⟨⟨i, o⟩, a⟩ := r
