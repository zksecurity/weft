...
|---|---|
| `Weft/Shape.lean` | `Domain ⟨share, clear⟩` (a share of a `T` and a clear `T`, per domain: `ideal`, `erase`, `timed`); clear values as an applicative; the closed `Shape` language with `interp` and `blank`, and `⊗` for products. |
