...
| `Weft/Std/Boolean.lean` | `GF2 := ZMod 2` with `ofBool`/`toBool`/`bitsOf`, and Boolean circuits `Bool2 := [Lin GF2, Mult GF2]` priced so that `delayOn` is the AND depth and `commOn` the AND count; `Bool2.model_eq`. |
